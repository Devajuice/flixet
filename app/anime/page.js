  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: "clamp(28px, 4vw, 48px)", fontWeight: "var(--font-extrabold)", letterSpacing: "-0.02em", lineHeight: 1, display: "flex", alignItems: "center", gap: 12 }}>
          <Sparkles size={28} color="var(--accent)" /> Anime Series
        </h1>
        <p style={{ fontSize: "var(--text-sm)", color: "var(--text-tertiary)", marginTop: 4 }}>Popular anime series from Japan</p>
      </div>

      {loading && animeList.length === 0 ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 16, marginBottom: 40 }}>
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} className="skeleton" style={{ borderRadius: "var(--radius-lg)", aspectRatio: "2/3" }} />
          ))}
        </div>
      ) : !animeList.length ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "40vh", gap: 12 }}>
          ...
          <p style={{ fontSize: "var(--text-sm)", color: "var(--text-tertiary)" }}>No anime found.</p>
        </div>
      ) : (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 16, marginBottom: 40 }}>
          ...
          <div ref={observerTarget} style={{ minHeight: 80, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 20 }}>
            {loadingMore && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                <div style={{ width: 28, height: 28, border: "3px solid rgba(229,9,20,0.2)", borderTopColor: "var(--accent)", borderRadius: "50%", animation: "spin 0.9s linear infinite" }} />
            ...
            {!hasMore && animeList.length > 0 && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "var(--text-sm)", color: "var(--text-tertiary)" }}>
                <Sparkles size={14} color="var(--accent)" /> {animeList.length} series loaded
              </div>
            )}
          </div>
        </>
