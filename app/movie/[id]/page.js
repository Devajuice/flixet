...
import { Calendar, Clock, Play, Star, ExternalLink, Film, Globe, Heart, ChevronRight } from "lucide-react";
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 var(--container-padding)" }}>
        <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 40, marginTop: -80, position: "relative", zIndex: 1 }}>
          ...
          <div style={{ paddingTop: 80 }}>
            ...
            {[100, 85, 70].map((w, i) => <div key={i} className="skeleton" style={{ height: 13, width: `${w}%`, marginBottom: 10, borderRadius: "var(--radius-md)" }} />)}
          </div>
        </div>
      </div>
    </div>
...
function InfoCard({ icon, label, value }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)" }}>
      ...
      <div>
        ...
        <p style={{ fontSize: "var(--text-sm)", fontWeight: "var(--font-semibold)", color: "var(--text-secondary)", margin: 0 }}>{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <Link href={`/person/${actor.id}`} style={{ display: "block" }}>
      <div
        ...
        style={{ width: 110, cursor: "pointer", borderRadius: "var(--radius-lg)", overflow: "hidden", background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", padding: 8, textAlign: "center", transition: "all var(--transition-base)", transform: hovered ? "translateY(-4px)" : "none" }}
      >
...
export default function MovieDetails({ params }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${API_KEY}&append_to_response=credits,videos,recommendations,watch%2Fproviders`),
      fetch(`https://api.themoviedb.org/3/movie/${movieId}/external_ids?api_key=${API_KEY}`),
    ])
      .then((rs) => Promise.all(rs.map((r) => r.json())))
      .then(([movieData, extData]) => {
        if (movieData.success === false || movieData.status_code) throw new Error(movieData.status_message || "Failed to fetch movie data");
        setMovie(movieData);
        if (OMDB_KEY && extData?.imdb_id) {
          fetch(`https://www.omdbapi.com/?i=${extData.imdb_id}&apikey=${OMDB_KEY}`)
            .then((r) => r.json())
            .then((d) => setImdbRating(d.imdbRating && d.imdbRating !== "N/A" ? { rating: d.imdbRating, votes: d.imdbVotes } : null))
  ...
  useEffect(() => {
    if (showPlayer && movie && !hasAdded.current) {
      addToContinueWatching({ id: movie.id, type: "movie", title: movie.title, poster_path: movie.poster_path, backdrop_path: movie.backdrop_path, runtime: movie.runtime || 120, progress: 15 });
  ...
  if (error || !movie)
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", textAlign: "center", padding: 20 }}>
        <p style={{ fontSize: "var(--text-2xl)", fontWeight: "var(--font-bold)", color: "var(--accent)", marginBottom: 12 }}>{error ? "Error Loading Movie" : "Movie Not Found"}</p>
        <p style={{ color: "var(--text-tertiary)", marginBottom: 24 }}>{error || "The movie you're looking for doesn't exist."}</p>
        <Link href="/movies"><button className="btn btn-primary">Back to Movies</button></Link>
      </div>
    );
  ...
  const recs = movie.recommendations?.results?.filter((m) => m.poster_path).slice(0, 16) || [];
  const directors = movie.credits?.crew?.filter((p) => p.job === "Director") || [];
  const writers = movie.credits?.crew?.filter((p) => p.job === "Screenplay")?.slice(0, 2) || [];
  const watchProviders = (movie["watch/providers"] ?? movie["watch%2Fproviders"])?.results?.IN;
  ...
  return (
    <>
      <VideoPlayer isOpen={showPlayer} onClose={() => setShowPlayer(false)} type="movie" id={movieId} title={movie.title} />
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 var(--container-padding)" }}>
        <div className="movie-detail-grid">
          ...
          <div style={{ position: "relative" }}>
            <div className="movie-poster-wrapper">
              <img src={poster} alt={`${movie.title} poster`} style={{ width: "100%", borderRadius: "var(--radius-xl)", boxShadow: "var(--shadow-xl)", display: "block" }} loading="lazy" />
            </div>
          </div>
          ...
          <div>
            ...
            <h1 style={{ fontSize: "clamp(28px, 4vw, 48px)", lineHeight: 1.1, fontWeight: "var(--font-extrabold)", letterSpacing: "-0.025em", color: "var(--text-primary)", marginBottom: 12 }}>{movie.title}</h1>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap", marginBottom: 28 }}>
              ...
              {imdbRating && (
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ fontSize: "var(--text-base)", fontWeight: "var(--font-bold)", color: "var(--gold)" }}>{imdbRating.rating}</span>
                  {imdbRating.votes && <span style={{ fontSize: "var(--text-xs)", color: "var(--text-tertiary)" }}>({imdbRating.votes})</span>}
                </div>
              )}
              <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                {movie.release_date && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--text-secondary)" }}>
                    <Calendar size={14} />
                    <span style={{ fontSize: "var(--text-sm)" }}>{new Date(movie.release_date).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</span>
                  </div>
                )}
                {movie.runtime && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--text-secondary)" }}>
                    <Clock size={14} />
                    <span style={{ fontSize: "var(--text-sm)" }}>{formatRuntime(movie.runtime)}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 32 }}>
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setShowPlayer(true)} className="btn btn-primary" style={{ padding: "clamp(10px, 2vw, 14px) clamp(20px, 4vw, 32px)", fontSize: "clamp(var(--text-sm), 2.5vw, var(--text-base))", fontWeight: "var(--font-bold)" }}>
              ...
              <WatchlistButton
                item={{ id: movie.id, type: "movie", title: movie.title, poster_path: movie.poster_path, vote_average: movie.vote_average, release_date: movie.release_date }}
                variant="large"
              />
            </div>
            ...
            {/* Overview */}
            {movie.overview && (
              <p style={{ fontSize: "clamp(var(--text-sm), 2.5vw, var(--text-base))", lineHeight: 1.85, color: "var(--text-secondary)", marginBottom: 32, maxWidth: 720 }}>{movie.overview}</p>
            )}

            {/* Metadata cards */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, marginBottom: 32 }}>
              {directors.length > 0 && <InfoCard icon={<Film size={18} />} label="Director" value={directors.map((d) => d.name).join(", ")} />}
              {writers.length > 0 && <InfoCard icon={<Globe size={18} />} label="Writer" value={writers.map((w) => w.name).join(", ")} />}
              {movie.original_language && <InfoCard icon={<Globe size={18} />} label="Language" value={new Intl.DisplayNames(["en"], { type: "language" }).of(movie.original_language) || movie.original_language} />}
              {movie.budget > 0 && <InfoCard icon={<Heart size={18} />} label="Budget" value={`$${(movie.budget / 1000000).toFixed(1)}M`} />}
              {movie.revenue > 0 && <InfoCard icon={<Star size={18} />} label="Revenue" value={`$${(movie.revenue / 1000000).toFixed(1)}M`} />}
              {movie.production_companies?.length > 0 && <InfoCard icon={<Film size={18} />} label="Studio" value={movie.production_companies.slice(0, 2).map((c) => c.name).join(", ")} />}
            </div>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              {movie.imdb_id && (
                <a href={`https://www.imdb.com/title/${movie.imdb_id}/`} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", background: "rgba(245,197,24,0.08)", border: "1px solid rgba(245,197,24,0.2)", borderRadius: "var(--radius-lg)", color: "var(--gold)", fontSize: "var(--text-sm)", fontWeight: "var(--font-medium)", textDecoration: "none" }}>
                  IMDb <ExternalLink size={13} />
                </a>
              )}
              {movie.homepage && (
                <a href={movie.homepage} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", color: "var(--text-secondary)", fontSize: "var(--text-sm)", fontWeight: "var(--font-medium)", textDecoration: "none" }}>
        ...
        {recs.length > 0 && (
          <ScrollRow title="More Like This" style={{ marginTop: 40, marginBottom: 60 }}>
            {recs.map((m) => <MediaCard key={m.id} item={m} type="movie" />)}
          </ScrollRow>
        )}

        {cast.length === 0 && recs.length === 0 && <div style={{ height: 60 }} />}
      </div>
